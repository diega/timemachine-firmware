...
{
    const mfactory_char_t *char_desc = mfactory_get_char((uint8_t)ch);
...
static esp_err_t max7219_driver_init(void)
{
    ...
    esp_err_t ret = max7219_init_desc(&s_dev, SPI_HOST, MAX7219_MAX_CLOCK_SPEED_HZ, MAX7219_PIN_CS);
    if (ret != ESP_OK) {
...
static void max7219_driver_render(const display_scene_t *scene)
{
    ...
    for (int i = 0; i < MAX7219_CASCADE; i++) {
        esp_err_t ret = max7219_draw_image_8x8(&s_dev, i, &s_display_buffer[i]);
        if (ret != ESP_OK) {
