...
/**
 * @brief MD_MAX72XX font data
 *
 * Original source: https://github.com/MajicDesigns/MD_MAX72XX
 * Copyright (C) 2012-14 Marco Colli. All rights reserved.
 * Licensed under LGPL 2.1
 *
 * This font uses the "new font" format from MD_MAX72XX.
 * The original array started with: 'F', 2, 0, 0, 0, 255, 8
 * Where:
 *   'F' (70) = Font file indicator
 *   2 = Version number
 *   0, 0 = First ASCII (16-bit)
 *   0, 255 = Last ASCII (16-bit)
 *   8 = Height in pixels
 *
 * We've stripped that header and kept only the character data.
 * Each character starts with its width byte, followed by that many column bytes.
 */
static const uint8_t MD_MAX72XX_FONT_DATA[] = {
    0,		// 0 - 'Empty Cell'
...
/**
 * @brief Parse character data from MD_MAX72XX format
 *
 * The MD_MAX72XX format stores each character as: [width_byte, col1, col2, ..., colN]
 * This function finds the character's data by walking through the array.
 */
const font_char_t* md_max72xx_font_get_char(uint8_t ch)
{
    ...
    // Walk through the font data to find the character
    uint16_t offset = 0;
    for (uint16_t i = 0; i <= ch; i++) {
        if (offset >= sizeof(MD_MAX72XX_FONT_DATA)) {
            // Past end of data
            char_desc.width = 0;
            char_desc.data = NULL;
            return &char_desc;
        ...
        uint8_t base_width = MD_MAX72XX_FONT_DATA[offset];
        ...
        if (i == ch) {
            ...
            const uint8_t *base_data = &MD_MAX72XX_FONT_DATA[offset + 1];
            for (int j = 0; j < base_width; j++) {
                char_data_with_spacing[j] = base_data[j];
            }
        ...
        // Skip to next character (width byte + width data bytes)
        offset += 1 + base_width;
    }
...
// Font descriptor for the MD_MAX72XX font
const font_t font_md_max72xx = {
    .name = "md_max72xx",
    .get_char = md_max72xx_font_get_char,
    .get_char_last = md_max72xx_font_get_char  // Same as regular get_char for MD_MAX72XX font
};
